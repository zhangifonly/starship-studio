...
综合演示与 Flight 5 上升近景共用 `src/pad-steam.ts`。白色可见云团表示
喷水降噪系统与高温排气作用后形成的凝结水滴，不把纯水蒸气本身画成白烟。
云团围绕各自的实际发射台坐标向四周铺开并向上翻滚，起飞后留在地面逐渐消散，
不会跟随火箭升空。综合演示从第 6 秒点火起生成；Flight 5 从离台开始展示，
因此预置 5 秒的点火云团。两套时间映射均为编排，不是实测排放或风场。

实例化软边云片按当前相机排序，保留设施深度遮挡，并在透明远景地面之后绘制，
避免地面把白云盖住。每团云只生成一次，位置和透明度由时间直接求值，支持
暂停、拖回和切换机位；未模拟流体力学、光线多重散射或真实天气。
`npm run test:steam` 检查生命周期、连续性、地面位置和回拖复原；
`node scripts/verify-steam.mjs` 检查两种演示、五个机位及五种屏幕尺寸，
加 `--webkit` 可检查 WebKit。截图与像素检查共同用于判断可见性。
