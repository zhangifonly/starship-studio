...
await mkdir('artifacts/ship30', { recursive: true });
...
try {
  for (const [width, height] of sizes) {
    ...
    await page.goto(`${base}/#mission/flight-5/overview?t=120&camera=ship-close`, { waitUntil: 'networkidle' });
    ...
    for (const camera of ['ship-close', 'ship-heat']) {
      ...
      let initial, splash;
      for (const t of [120, 130, 140, 146, 155, 160, 164, 170, 179, 180, 200, 120]) {
        ...
        await page.screenshot({ path: `artifacts/ship30/${engine.name()}-${width}-${camera}-${t}.png`, fullPage: true });
        expect(colors.size, `${camera} at ${t}s / ${width}px main-view pixels`).toBeGreaterThan(30);
        if (t === 120) { if (initial) expect(Buffer.compare(initial, buffer)).toBe(0); else initial = buffer; }
        if (t === 180) splash = buffer; if (t === 200) expect(Buffer.compare(splash, buffer)).toBe(0);
      }
    }
    await slider.fill('146'); await expect(canvas).toHaveAttribute('data-time', '146.00');
    ...
    await slider.fill('119.9'); await expect(canvas).toHaveAttribute('data-ship-near', 'false');
    await slider.fill('120'); await expect(canvas).toHaveAttribute('data-ship-near', 'true');
    if (!visualOnly) {
      await slider.fill('159.5'); await page.getByRole('button', { name: '播放全程复盘', exact: true }).click();
      await expect.poll(async () => Number(await canvas.getAttribute('data-time'))).toBeGreaterThan(160.4);
      await expect(page.getByTestId('overview-audio')).toHaveAttribute('src', /flight5-overview-9-/);
