...
test('stack starts over the preserved support deck, with both stages sharing one datum', () => {
  ...
  for (let t = 0; t <= 160; t += .25) assert.deepEqual(boosterPositionAt(t), shipPositionAt(t));
});
...
test('hot-stage collar bridges B12 dome and ship skirt; close-ups stop before separation', () => {
  assert.ok(ASCENT_RING_BOTTOM < 7.07 && ASCENT_RING_TOP > 7.07);
  assert.ok(ASCENT_SHIP_Y > 7.07); assert.ok(Math.abs(ASCENT_SHIP_Y + .11 - ASCENT_RING_TOP) < .03);
  assert.equal(ascentCameraAvailable(159.9), true); assert.equal(ascentCameraAvailable(160), false); assert.equal(ascentCameraAvailable(NaN), false);
