...
export default function MissionView({ onFullscreen }: { onFullscreen: () => void }) {
  ...
  return <section className="mission-workspace" aria-label="历史任务复盘">
    ...
    <div className="mission-topbar"><div><div className="mission-panel-eyebrow">任务档案 / 005</div><h1>Flight 5 <span>捕获复盘</span></h1></div><div className="mission-identity"><span><i/>历史任务 · 三维重建</span><small>Booster 12 / Pad A / 2024.10.13</small></div><label className="mission-picker"><select aria-label="任务选择" value="flight-5" onChange={e => { if (e.target.value === 'concept') location.hash = 'launch'; }}><option value="flight-5">Flight 5 · 历史捕获</option><option value="concept">综合流程 · 概念演示</option></select></label><button className="tool mission-evidence-button" aria-label="查看任务资料" title="任务资料" onClick={() => { setPlaying(false); evidence.current?.showModal(); }}><FileText size={18}/></button></div>
