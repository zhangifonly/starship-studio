  ...
  return <section className="launch-workspace" aria-label="发射演示">
    ...
    <div className="launch-main">
      <div className="launch-scene-area">
        ...
        <label className="launch-camera"><Camera size={14}/><select aria-label="发射视角" value={following ? cameraMode : 'manual'} onChange={e => { setCameraMode(e.target.value as CameraMode); resetCamera(); }}>{!following && <option value="manual" disabled>自由视角</option>}{cameraModes.map(mode => <option key={mode.id} value={mode.id}>{mode.name}</option>)}</select></label>
