...
import { createGridFins } from './grid-fins';
...
export function createCaptureBooster() {
  ...
  const gridFins = createGridFins(edge, dark), fins = gridFins.root; root.add(fins);
  ...
  return { root, pins, pinHighlights, skin, fins, engines, dome, updateGridFins: gridFins.update };
}
