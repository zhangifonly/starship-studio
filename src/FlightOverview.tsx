...
function OverviewEvidence({ index }: { index: number }) {
  ...
  return <div className="mission-panel-body overview-evidence-body">
    ...
    <dl className="mission-limits"><div><dt>发射近景边界</dt><dd>起飞至分离前展示 B12、热分离环和第一代 S30。起点与发射台裙部支座共用坐标；先垂直离塔，再转向下程。加速、俯仰、发动机关机过渡及蒸汽为示意，不是遥测或流场计算。双臂与脐带臂已避让，不重演装配流程。长焦观察点未经实拍标定；分离开始切回区域视图，热分离近景尚未重建。</dd></div></dl>
...
export default function FlightOverview({ onFullscreen }: { onFullscreen: () => void }) {
  ...
  return <section className="mission-workspace overview-workspace" aria-label="Flight 5 全程航迹">
    ...
    <div className="mission-topbar"><div><div className="mission-panel-eyebrow">任务档案 / 005</div><h1>Flight 5 <span>全程航迹</span></h1></div><div className="mission-identity"><span><i/>历史事件 · 示意航迹</span><small>B12 + S30 / 2024.10.13</small></div><label className="mission-picker"><select aria-label="任务选择" value="overview" onChange={e => { location.hash = e.target.value === 'capture' ? 'mission/flight-5' : 'launch'; }}><option value="overview">Flight 5 · 全程航迹</option><option value="capture">Flight 5 · 捕获特写</option><option value="concept">综合流程 · 概念演示</option></select></label><button className="tool mission-evidence-button" aria-label="查看航迹资料" title="航迹资料" onClick={() => { setPlaying(false); evidence.current?.showModal(); }}><FileText size={18}/></button></div>
    <div className="mission-body"><div className="mission-viewer">
      ...
      <div className={`mission-stage overview-stage${closeView ? ' return-stage' : ''}`}>
        ...
        <div className="overview-legend">{ascentView ? <><span>B12 + S30 / 第一代</span><span>{camera === 'ascent-ground' ? '固定地面机位 · 自动焦距' : '组合体上升跟踪'}</span></> : shipView ? <><span>S30 / 第一代 · 四襟翼</span><span>{shipPose.phase}</span></> : returnView ? <><span>B12 / 四栅格翼</span><span>{camera === 'ground' ? '固定地面机位 · 自动焦距' : '返回近距跟踪'}</span></> : <><span><i className="ship-key"/>S30 上面级</span><span><i className="booster-key"/>B12 助推器</span></>}</div>
        ...
        <span className="overview-render-note">{ascentView ? '上升与尾焰重建 · 非遥测' : isAscentCamera(camera) ? '分离后 · 发射场区域' : shipView ? state.splashed ? '入水瞬间定格 · 后续未重建' : 'S30 姿态与受热重建 · 非遥测' : returnView ? state.seconds > 426.6 ? '捕获状态保留 · 后续转运未重建' : 'B12 返回重建 · 非实拍机位' : isReturnCamera(camera) ? '抛环前 · 发射场区域' : isShipCamera(camera) ? '关机前 · 上面级区域' : '示意插值 · 非遥测'}</span>
      </div>
