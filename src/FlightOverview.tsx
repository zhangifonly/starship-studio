...
import { flight5State, formatMissionClock, overviewCameras, overviewEventAt, overviewEvents, overviewHash, OVERVIEW_DURATION, parseOverviewHash, STARBASE, timelineSources, type OverviewCamera } from './flight5-timeline';
...
export default function FlightOverview({ onFullscreen }: { onFullscreen: () => void }) {
  ...
  return <section className="mission-workspace overview-workspace" aria-label="Flight 5 全程航迹">
    ...
    <div className="mission-body"><div className="mission-viewer">
      ...
      <div className="mission-stage overview-stage">
        ...
        <div className="overview-legend"><span><i className="ship-key"/>S30 上面级</span><span><i className="booster-key"/>B12 助推器</span></div>
        <div className="overview-inset-label">STARBASE <span>{state.caught ? 'B12 已捕获' : state.separated ? 'B12 返回中' : '两级上升'}</span></div>
        ...
        <span className="overview-render-note">示意插值 · 非遥测</span>
      </div>
