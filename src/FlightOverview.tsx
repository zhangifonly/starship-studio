...
function OverviewEvidence({ index }: { index: number }) {
  ...
  return <div className="mission-panel-body overview-evidence-body">
    ...
    <dl className="mission-limits"><div><dt>S30 近景边界</dt><dd>约 T+03:43 起展示第一代外观，四片襟翼、三台海平面与三台真空发动机。六机动力飞行在约 T+08:27 关机后衔接滑行；最后四秒的尾焰淡出是视觉编排，不代表逐台关机记录。热盾分布、姿态、襟翼、尾焰、受热发光与海浪为重建，不是遥测或流场计算；没有复原再入损伤。夜侧有展示补光，机位不是实拍摄影点。画面停在入水瞬间；入水后的火球与后续状态未重建。</dd></div></dl>
  ...
  return <section className="mission-workspace overview-workspace" aria-label="Flight 5 全程航迹">
    ...
    <div className="mission-body"><div className="mission-viewer">
      ...
      <div className={`mission-stage overview-stage${closeView ? ' return-stage' : ''}`}>
        ...
        <span className="overview-render-note">{ascentView ? state.seconds >= 159 ? '热分离与泄流示意 · 非遥测' : '上升与尾焰重建 · 非遥测' : isAscentCamera(camera) ? '返航点火后 · 发射场区域' : shipView ? state.splashed ? '入水瞬间定格 · 后续未重建' : state.seconds < 507 ? 'S30 动力段与尾焰重建 · 非遥测' : 'S30 姿态与受热重建 · 非遥测' : returnView ? camera === 'fins' ? '栅格翼控制示意 · 非遥测角度' : state.seconds > 426.6 ? '捕获状态保留 · 后续转运未重建' : 'B12 返回重建 · 非实拍机位' : isReturnCamera(camera) ? '抛环前 · 发射场区域' : isShipCamera(camera) ? '近景起点前 · 上面级区域' : '示意插值 · 非遥测'}</span>
      </div>
